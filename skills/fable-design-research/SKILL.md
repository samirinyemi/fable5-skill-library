...
| **Awwwards** | https://www.awwwards.com/ | Award-tier full sites & live interactions | `https://www.awwwards.com/websites/?text=QUERY` · category pages e.g. `/inspiration/scroll-animations` · **scripted, no browser needed:** `bash scripts/awwwards-live-urls.sh [limit] [query] [gallery-path]` (in this skill's folder) returns `slug \| live-site-URL` pairs — default gallery = winners; pass `"/websites/nominees/"` for the freshest daily submissions |
